...
main (int argc, char *argv[])
{
  ...
  //LogComponentEnable ("LtePhy", LOG_LEVEL_FUNCTION);
  
  //uint16_t numberOfNodes = 2;
  uint16_t numberOfues = 10;
  double simTime = 1440;  // mins JMS edit 10.12
  ...
  enbphy = enbNodes.Get(0)->GetObject<LteEnbPhy>();
  std::cout<<"The ptr of enb is "<<enbphy<<std::endl;
  
  /*
  //print the default downlink bandwidth
  Ptr<LteEnbNetDevice> enbdev = enbNodes.Get(0)->GetObject<LteEnbNetDevice>(); //9.16
  UintegerValue dlBW; //9.16
  std::cout<<"until here"<<std::endl;
  enbdev->GetAttribute("DlBandwidth",dlBW); //9.16
  //std::cout<<"until here"<<std::endl;
  NS_LOG_INFO("The default DlBandwidth of LteEnbNetDevice is "<<dlBW.Get()); //9.16
  */
  ...
  lteHelper->EnableDlPacketTraces (); //JMS add
  // Uncomment to enable PCAP tracing
  //p2ph.EnablePcap("test", internetDevices.Get(0), true, true); //对pgw进行抓包统计

  Simulator::Stop(Minutes(simTime)); //JMS edit 10.12 from Seconds() to Minutes()
  Simulator::Run();
