      ...
      if (m_closeOnEmpty == false)
        {
          ...
          //std::cout<<"Socket "<<this<<" deferring close,state "<< TcpStateName[m_state]<<std::endl;
        }
      return 0;
    }
  return DoClose ();
      ...
      if (m_state == ESTABLISHED || m_state == CLOSE_WAIT)
        {
          ...
          if (m_state == ESTABLISHED)
            ...
            { // On passive close: Peer sent me FIN already
              ...
              std::cout<<"TcpSocketBase ("<< this <<" )"<<":: ShutdownSend(): CLOSE_WAIT -> LAST_ACK "<<std::endl;  //JMS
              m_state = LAST_ACK;
            }  
        }
    }
  ...
  if (m_state == ESTABLISHED || m_state == SYN_SENT || m_state == CLOSE_WAIT)
    {
      ...
      if (!m_txBuffer->Add (p))
        { // TxBuffer overflow, send failed
          ...
          std::cout<<"ERROR_MSGSIZE---TxBuffer overflow, send failed"<<std::endl;
          return -1;
        }
      if (m_shutdownSend)
        {
          m_errno = ERROR_SHUTDOWN;
          std::cout<<"ERROR_SHUTDOWN "<<std::endl;
          return -1;
        }
    ...
    { // Connection not established yet
      ...
      std::cout<<"ERROR_NOTCONN---Connection not established yet"<<std::endl;
...
{
  NS_LOG_FUNCTION (this);
  //std::cout<<"TcpSocketBase::GetTxAvailable():------"<<m_txBuffer->Available ()<<std::endl;
...
TcpSocketBase::DoClose (void)
{
  ...
  switch (m_state)
    ...
    case ESTABLISHED:
      ...
      SendEmptyPacket (TcpHeader::FIN);
      NS_LOG_INFO ("ESTABLISHED -> FIN_WAIT_1");
      std::cout<<"TcpSocketBase::DoClose(): case ESTABLISHED"<<std::endl;
      m_state = FIN_WAIT_1;
      break;
    case CLOSE_WAIT:
      ...
      NS_LOG_INFO ("CLOSE_WAIT -> LAST_ACK");
      std::cout<<"TcpSocketBase ("<< this <<" )"<<":: DoClose(): CLOSE_WAIT -> LAST_ACK "<<std::endl;  //JMS
      m_state = LAST_ACK;
      break;
    case SYN_SENT:
    case CLOSING:
...
TcpSocketBase::CloseAndNotify (void)
{
  ...
  std::cout<<"TcpSocketBase ("<< this <<" )"<<"::CloseAndNotify() "<< TcpStateName[m_state]<<" ->CLOSED"<<" already"<<std::endl; // JMS
  m_state = CLOSED;
...
TcpSocketBase::ReceivedAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  if (0 == (tcpHeader.GetFlags () & TcpHeader::ACK))
    ...
    { // Case 3: New ACK, reset m_dupAckCount and update m_txBuffer
      ...
      //std::cout<<"TcpSocketBase::ReceivedAck()--------tcpHeader.GetAckNumber:"<<tcpHeader.GetAckNumber ()<<std::endl;
                              ...
                              const Address& fromAddress, const Address& toAddress)
{
  ...
  Ptr<TcpSocketBase> newSock = Fork (); //JMS Fork()returns a copy of the socket
    ...
    { // Process the ACK, and if in FIN_WAIT_1, conditionally move to FIN_WAIT_2
      ...
      if (m_state == FIN_WAIT_1 && m_txBuffer->Size () == 0
        ...
        { // This ACK corresponds to the FIN sent
          std::cout<<"TcpSocketBase( "<< this <<" )"<<"::ProcessWait(..): FIN_WAIT_1 -> FIN_WAIT_2"<<std::endl; //JMS
...
TcpSocketBase::DoPeerClose (void)
{
  ...
  std::cout<<"TcpSocketBase ("<< this <<" )"<<":: DoPeerClose(): "<< TcpStateName[m_state] <<" ->CLOSE_WAIT "<<std::endl;  //JMS
  m_state = CLOSE_WAIT;
  ...
  if (m_closeOnEmpty && (remainingData == 0))
    {
      ...
      if (m_state == ESTABLISHED)
        { // On active close: I am the first one to send FIN
          NS_LOG_INFO ("ESTABLISHED -> FIN_WAIT_1");
          std::cout<<"TcpSocketBase ("<< this <<" )"<<":: sendDataPacket(): ESTABLISHED-> FIN_WAIT_1 "<<std::endl;  //JMS 
          m_state = FIN_WAIT_1;
        }
      else if (m_state == CLOSE_WAIT)
        { // On passive close: Peer sent me FIN already
          NS_LOG_INFO ("CLOSE_WAIT -> LAST_ACK");
          std::cout<<"TcpSocketBase ("<< this <<" )"<<":: sendDataPacket(): CLOSE_WAIT -> LAST_ACK "<<std::endl;  //JMS
          m_state = LAST_ACK;
        }
    }
  TcpHeader header;
...
{
  std::cout<<"TcpSocketBase ("<< this <<" )"<<"::TimeWait() "<< TcpStateName[m_state]<< " -> TIME_WAIT "<<std::endl;  //JMS
