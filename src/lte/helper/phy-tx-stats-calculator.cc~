...
PhyTxStatsCalculator::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::PhyTxStatsCalculator")
    ...
    .AddAttribute ("UlTxOutputFilename",
                   ...
                   MakeStringChecker ())
    .AddAttribute ("DlPacketsFilename",
                   "Name of the file where the downlink packet status results will be saved.",
                   StringValue ("DlpacketsJMS.txt"),
                   MakeStringAccessor (&PhyTxStatsCalculator::SetDlPktOutFilename),
                   MakeStringChecker ())
...
//JMS add to set the outfile name of dlpkt
void
PhyTxStatsCalculator::SetDlPktOutFilename (std::string outputFilename)
{
  LteStatsCalculator::SetDlPktFilename (outputFilename); //haven't finished
}

std::string
PhyTxStatsCalculator::GetDlPktOutFilename (void)
{
  return LteStatsCalculator::GetDlPktFilename ();
}
//JMS add finish
...
//JMS add the sink function for m_dlPackets in lte-enb-phy
void
PhyTxStatsCalculator::DlPacketsCallback (Ptr<PhyTxStatsCalculator> phyTxStats,
                      std::string path, PacketsFromMacToPhy params)
{
    NS_LOG_FUNCTION (phyTxStats);
    std::cout<<"PhyTxStatsCalculator::DlPacketsCallback()-----in the sink static function for the dlpkt trace source!"<<std::endl;
    phyTxStats->DlPackets (params);
...
PhyTxStatsCalculator::DlPackets(PacketsFromMacToPhy params)
{
    ...
    {
      outFile.open (GetDlPktOutFilename ().c_str ());
      if (!outFile.is_open ())
        {
          NS_LOG_ERROR ("Can't open file " << GetDlPktOutFilename ().c_str ());
          return;
        }
      m_dlpktFirstWrite = false;
      
      outFile << "% time\tbytesOfnewpacket";
      outFile << std::endl;
    }
    
    else
    {
      outFile.open (GetDlPktOutFilename ().c_str (),  std::ios_base::app);
      if (!outFile.is_open ())
        {
          NS_LOG_ERROR ("Can't open file " << GetDlPktOutFilename ().c_str ());
          return;
    ...
    outFile << params.m_time << "\t";
    outFile << params.m_packet << std::endl;
