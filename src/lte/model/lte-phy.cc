...
{
  NS_LOG_FUNCTION (this);
  std::cout<<"LtePhy---LtePhy(Ptr<LteSpectrumPhy> dlPhy, Ptr<LteSpectrumPhy> ulPhy)"<<std::endl;
}
...
LtePhy::SetMacPdu (Ptr<Packet> p)
{
  ...
  //std::cout<<"Ltephy("<< this<<")"<<"::SetMacPdu(Ptr<Packet> p)----add a packet to the queue,size: "<< p->GetSize()<< 
  //" at "<<Simulator::Now ().GetSeconds ()<<std::endl; // JMS
