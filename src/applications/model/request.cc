...
Request::StartApplication (void)
{
  ...
  m_socCreateEvent = Simulator::Schedule (Seconds (2), &Request::randomSocketCreat, this); //todo: 开始时间随机 
...
void Request::HandlePeerClose (Ptr<Socket> socket)
{
  ...
  for (iter = LinkList.begin(); iter != LinkList.end(); ++iter)
  {
      ...
      {
          std::cout<<"There's a socket in m_socketList, going to delete it"<<std::endl;
          break;
      }        
  }
  ...
  {
       std::cout<<"There is no inter->socket == socket in m_socketList!"<<std::endl;
...
void randomSocketCreat()
{
  ...
  //m_socCreateEvent = Simulator::Schedule (Seconds (2), &Request::randomSocketCreat, this); //wait and send another request
