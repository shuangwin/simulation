                ...
                case '1':
                    {
                        ...
                        linknew.contentBytes = 1024*100; //coast 100k，10s，至少的youtube视频
                        break;     
                    }
                 
                case '2':
                    {
                        linknew.contentBytes = 1024*1024*30; //30M
                        break;    
                    }
                case '3':
                    { 
                        linknew.contentBytes = 1024*1024*500; //500M
                        break;
                    }
...
{
  NS_LOG_FUNCTION (this << socket);
  //JMS 10.20 when UE close first,then close the relate socket in linklist and accepted socket list...start
  ...
  for (socket_iter = m_socketList.begin(); socket_iter != m_socketList.end(); ++socket_iter)
  {
       ...
       if(socket_iter == m_socketList.end() && *socket_iter!= socket)
       {
          std::cout<<"There is no *socket_iter == socket in m_socketList!"<<std::endl;
  ...
  for (iter = LinkList.begin(); iter != LinkList.end(); ++iter)
  {
      ...
      {
          std::cout<<"There's a socket in LinkList to be closed!"<<std::endl;
          LinkList.erase(iter);
          break;
      }        
  }
  ...
  {
       std::cout<<"There is no inter->socket == socket in LinkList to be closed!"<<std::endl;
       return;  //return语句！ 强制结束当前函数
  }
  ...
  //JMS 10.20 when UE close first,then close the relate socket in linklist and accepted socket list...end 
...
void ContentServer::SendData(Ptr<Socket> socket)
{
  ...
  while(iter->totBytes < iter->contentBytes)
    ...
    {
        //JMS waiting for user to receive all and close the socket
        std::cout<<"Server has finished sending to socket"<<iter->socket<<std::endl;
    }
