 ...
 */
class ContentServer : public Application 
{
public:
  /**
...
private:
  ...
  //Ptr<Socket>     send_socket;   //equals to the socket which is sending content to users
  typedef struct _link
  
  {
    Ptr<Socket>     socket;
    uint32_t        contentBytes; //set dynamiclly to be the total bytes of different contents
    uint32_t        totBytes;  //!< Total bytes sent so far 
  }Link;
