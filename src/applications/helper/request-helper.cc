 ...
 */
#include "request-helper.h"
#include "ns3/request.h"
...
RequestHelper::RequestHelper (Address address, uint16_t port)
{
  m_factory.SetTypeId (Request::GetTypeId ());
  SetAttribute ("RemoteAddress", AddressValue (address));
  SetAttribute ("RemotePort", UintegerValue (port));
}

RequestHelper::RequestHelper (Ipv4Address address, uint16_t port)
{
  m_factory.SetTypeId (Request::GetTypeId ());
...
RequestHelper::RequestHelper (Ipv6Address address, uint16_t port)
{
  m_factory.SetTypeId (Request::GetTypeId ());
  SetAttribute ("RemoteAddress", AddressValue (Address(address)));
  SetAttribute ("RemotePort", UintegerValue (port));
...
void
RequestHelper::SetAttribute (std::string name, const AttributeValue &value)
{
  m_factory.Set (name, value);
}

ApplicationContainer
RequestHelper::Install (NodeContainer c)
{
  ApplicationContainer apps;
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      Ptr<Node> node = *i;
      Ptr<Request> client = m_factory.Create<Request> ();
